...
class Base:
    ...
    def get_reset_data(self, batch_size):
        ...
        data = dict(
            ...
            goals=np.full(
                ((batch_size, np.prod(self.state_dim))), np.nan, dtype=np.float32
            ),
            rewards=np.full((batch_size, 1), np.nan, dtype=np.float32),
            terminals=np.full((batch_size, 1), np.nan, dtype=np.float32),
            hl_logprobs=np.full((batch_size, 1), np.nan, dtype=np.float32),
            ll_logprobs=np.full((batch_size, 1), np.nan, dtype=np.float32),
            hl_entropys=np.full((batch_size, 1), np.nan, dtype=np.float32),
            ll_entropys=np.full((batch_size, 1), np.nan, dtype=np.float32),
            masks=np.full((batch_size, 1), np.nan, dtype=np.float32),
        )
        return data
    ...
    def collect_trajectory(
        ...
        while current_step < self.min_batch_for_worker:
            ...
            for t in range(self.episode_len):
                with torch.no_grad():
                    [goal, a], metaData = policy(state, deterministic=deterministic)
                ...
                data["goals"][current_step + t] = goal
                data["rewards"][current_step + t] = rew
                data["terminals"][current_step + t] = done
                data["hl_logprobs"][current_step + t] = (
                    metaData["hl_logprobs"].cpu().detach().numpy()
                )
                data["ll_logprobs"][current_step + t] = (
                    metaData["ll_logprobs"].cpu().detach().numpy()
                )
                data["hl_entropys"][current_step + t] = (
                    metaData["hl_entropy"].cpu().detach().numpy()
                )
                data["ll_entropys"][current_step + t] = (
                    metaData["ll_entropy"].cpu().detach().numpy()
                )
                data["masks"][current_step + t] = metaData["mask"]
